...
@Component({
  ...
  template: `
    <app-layout 
      title="signup.components.emailVerification.title" 
      instructions="signup.components.emailVerification.instructions" 
      [params]="{emailAddress: emailAddress}">
      
      <form class="form-custom mt-3" [formGroup]="emailVerificationForm" (ngSubmit)="onSubmit()" novalidate>
        ...
        <div class="mt-5 d-grid gap-2 d-md-flex justify-content-between mt-3">
          ...
          <button class="btn btn-success rounded-pill shadow-none" type="submit" [disabled]="emailVerificationForm.invalid">{{ 'signup.common.next' | translate }}</button>
        </div>
      </form>
    </app-layout>
  `,
...
export class EmailVerificationComponent implements OnInit {
  ...
  get code(): FormArray {
    return this.emailVerificationForm.get('code') as FormArray;
  }
  ...
  onPaste(event: ClipboardEvent) {
    ...
    if (pastedText && /^\d{6}$/.test(pastedText)) {
      ...
      this.emailVerificationForm.updateValueAndValidity();
    }
